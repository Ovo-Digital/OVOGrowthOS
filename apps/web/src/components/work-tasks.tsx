...
import { TaskHourPanel } from '@/components/work-planning';
...
export function WorkTasks({ brandId }: { brandId?: string }) {
  ...
  const completion = useMutation({ mutationFn: (task: Work) => api(`/api/work-tasks/${task.id}/completion`, { method: 'PUT', body: JSON.stringify({ completed: !task.completedAt, revision: task.revision }) }),
    onSuccess: (_, task) => { setMessage(task.completedAt ? 'Görev yeniden açıldı.' : 'Görev tamamlandı. Finansal dönem veya anlaşma durumu değiştirilmedi.'); cache.invalidateQueries({ queryKey: ['work-tasks'] }); cache.invalidateQueries({ queryKey: ['tasks'] }); cache.invalidateQueries({ queryKey: ['work-planning'] }); cache.invalidateQueries({ queryKey: ['task-hour-plan'] }); }, onError: (error) => setMessage(error.message) });
  ...
  return <Card className="mt-5 p-5">
    ...
    {planningTaskId && <TaskHourPanel key={planningTaskId} taskId={planningTaskId} canManage={canManage} onClose={() => setPlanningTaskId(null)} />}
    {editing && brandId && <TaskForm key={editing === 'new' ? 'new' : `${editing.id}-${editing.revision}`} brandId={brandId} task={editing === 'new' ? undefined : editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); setMessage('Görev kaydedildi.'); cache.invalidateQueries({ queryKey: ['work-tasks'] }); cache.invalidateQueries({ queryKey: ['tasks'] }); cache.invalidateQueries({ queryKey: ['work-planning'] }); cache.invalidateQueries({ queryKey: ['task-hour-plan'] }); }} />}
    {result.isPending ? <p role="status">İşler yükleniyor…</p> : result.isError ? <div role="alert">{result.error.message} <button className="underline" onClick={() => result.refetch()}>Yeniden dene</button></div> : view === 'approvals' ? <div className="space-y-3">{approvals.data?.items.map(item => <div className="rounded-lg border p-4" key={item.id}><Link className="font-semibold underline" href={`/performance/${item.id}`}>{item.brandName} · {item.month}/{item.year}</Link><p className="mt-1 text-sm">Hazırlayan: {item.preparedBy || 'Kayıtlı değil'}. Hazırlayan dışında bir yönetici veya iş ortağı onaylamalıdır.</p></div>)}</div> : <div className="space-y-3">{tasks.data?.items.map(task => <article className="rounded-lg border p-4" key={task.id}>
      ...
      <div className="mt-3 flex flex-wrap gap-3 text-sm">{canManage && !task.completedAt && <button className="underline" onClick={() => { if (brandId) setEditing(task); }} disabled={!brandId}>{brandId ? 'Düzenle' : 'Düzenlemek için markayı açın'}</button>}{(canManage || task.assigneeId === me.data?.id) && <button className="rounded-lg border px-3 py-1.5 disabled:opacity-50" disabled={completion.isPending} onClick={() => completion.mutate(task)}>{task.completedAt ? 'Yeniden aç' : 'Tamamlandı olarak işaretle'}</button>}<button className="underline" disabled={!!planningTaskId} onClick={() => setPlanningTaskId(task.id)}>Haftalık saat planı</button><Link className="underline" href={`/activity?entityType=WorkTask&entityId=${task.id}`}>İşlem geçmişi</Link></div>
    </article>)}</div>}
