...
25 Eylül 2026: `20260925183737_BrandMailPolicy` yalnız `growth.BrandMailPolicies` tablosunu ekler. Marka başına tek kayıt, mevcut markaya zorunlu ilişki, pozitif concurrency sürümü ve sınırlı uzunlukta düz metin şablonlar vardır. Kayıt yoksa rapor e-postası izni kapalıdır; migration hiçbir marka iznini açmaz veya kişisel tercihi değiştirmez. Şema private kalır. Migration build/test sonrasında Supabase'e uygulandı; gerçek PostgreSQL ilişki/biriciklik/sürüm testleri ve API ayar okuması geçti. Deneme verileri geri alındı. Gönderim sırasında marka satırı paylaşım kilidiyle, izin değişikliğinde güncelleme kilidiyle korunur. Eski API marka kuralını tanımadığından karma sürümde gönderim yapılmamalıdır; geri dönüşte tablo korunur ve gönderim kapalı tutulur. Veri silen `Down` rutin geri alma adımı değildir.
...
25 Eylül 2026: `20260925231801_BrandReportSchedule` yalnız `growth.BrandMailPolicies` tablosuna üç sütun ve bir kontrol kısıtı ekler: `ScheduledReportEnabled`, `ScheduledSendDay` (1–31) ve `ScheduledSendHour` (0–23). Mevcut kayıtlara sırasıyla `false`, `5` ve `9` değeri yazılır; migration hiçbir markanın zamanlanmış gönderimini kendiliğinden açmaz, kişisel tercihlere dokunmaz. Zamanlama ayrı bir tablo değil, marka e-posta kuralının parçasıdır; bekleyen gönderim `growth.UserNotifications` içinde durur ve kişi/olay biricikliği mükerrer iletiyi engiller. Migration build/test sonrasında Supabase'e uygulandı; bekleyen migration veya model farkı kalmadı. Geri dönüşte eklenen sütunlar ve kısıt silinir, veri satırı korunur.
